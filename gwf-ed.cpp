	...
	while (kdq_size(A)) //// while there are still diagonals to update the wavefront
	{
		...
		{ // the most common case: the wavefront is in the middle
			...
			if (i >= 0 && k >= 0)
			{
				...
				if (diag_map.count(vd_to) == 0 || diag_map[vd_from].s + 1 < diag_map[vd_to].s)
				{
					...
					tb_expand(diag, CigarOperation::DELETION, v, v, i, k, k + 1);
					diag_map[vd_to] = diag;
				}
			}
			if (i >= 0 && k >= 0)
			{
				...
				{
					diag = diag_map[vd_from];
					tb_expand(diag, CigarOperation::INSERTION, v, v, i + 1, k, k);
					diag_map[vd_to] = diag;
				}
			}
			if (d == 0 || (i >= 0 && k >= 0))
			{
				//// TB: Mismatch
				vd_to = vd_from;
				tb_expand(diag_map[vd_to], CigarOperation::MISMATCH, v, v, i + 1, k, k + 1);
			}
		}
		...
		{					 // k + 1 == g->len[v]; reaching the end of the vertex but not the end of query
			...
			{											 // traverse $v's neighbors
				...
				{															  //// EXTENSION ACROSS VERTICES
					...
					if (absent)
					{
						...
						{
							diag = diag_map[vd_from];
							tb_expand(diag, CigarOperation::MATCH, v, w, i + 1, k, ol);
							diag_map[vd_to] = diag;
						}
					}
				}
				else if (absent) //// EXPANSION ACROSS VERTICES
				{
					...
					{
						diag = diag_map[vd_from];
						tb_expand(diag, CigarOperation::DELETION, v, w, i, k, ol);
						diag_map[vd_to] = diag;
					}
					...
					{
						diag = diag_map[vd_from];
						tb_expand(diag, CigarOperation::MISMATCH, v, w, i + 1, k, ol);
						diag_map[vd_to] = diag;
					}
				}
			}
			...
			{							// add an insertion to the target; this *might* cause a duplicate in corner cases
				...
				{
					diag = diag_map[vd_from];
					tb_expand(diag, CigarOperation::INSERTION, v, v, d + 1 + k, k, k);
					diag_map[vd_to] = diag;
				}
			}
		}
		...
		{																   // i + 1 == ql; reaching the end of the query but not the end of the vertex
			...
			{
				diag = diag_map[vd_from];
				tb_expand(diag, CigarOperation::DELETION, v, v, d + k, k, k + 1);
				diag_map[vd_to] = diag;
			}
		}
		else if (v != v1)
